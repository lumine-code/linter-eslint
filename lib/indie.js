...
class ProjectLinter {
  ...
  async isPathIgnored(filePath, ignoreGlob, ignoreVCS) {
    ...
    return picomatch.isMatch(normalizedFilePath, ignoreGlob);
  }

  async filterIgnoredResults(results) {
    const ignoreGlob = lumine.config.get("linter.ignoreGlob");
    const ignoreVCS = lumine.config.get("core.excludeVcsIgnoredPaths");
  ...
  runScan(scanItems = null) {
    ...
    const useBuiltin = lumine.config.get("linter-eslint.useBuiltin");
